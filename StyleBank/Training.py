...
args.CONTENT_IMG_DIR = '../Images/original'
args.STYLE_IMG_DIR = '../Images/style-images'
args.MODEL_WEIGHT_DIR = 'weights_test'
...
Training
"""
# 一个style图对应一个style id
# [0, 1, 2, ..., N]
style_id = list(range(len(style_dataset)))
style_id_idx = 0
# style也不是一次训练一张，由于我们有很多style，因此一次是训练一批style
# style_id_seg记录了一个batch的style bank
style_id_seg = []
# 根据batch size对style id进行分段
...
while global_step <= args.MAX_ITERATION:
    for i, data in enumerate(content_dataloader):
        ...
        if global_step % (args.T+1) != 0:
            # 训练style bank
            style_id_idx += 1
            # 选择对应的style id segment，content batch size中的每张图对应一个style id
