...
class Vgg19(torch.nn.Module):
    ...
    使用了VGG19的预训练模型，并且只提取上面说的几个特定层的feature map
    """
    def __init__(self, requires_grad=False, show_progress=False, use_relu=True):
        ...
        # 所有我们关注的layer，用于style representation，因此排除了conv4_2
        # 这用于 style representation，用名字取索引forward出来的结果
    ...
    def forward(self, x):
        # 一个block一个block的进行卷积操作，不过由于这个是预训练好的模型，我们只关注的特定几个层的feature map
        x = self.slice1(x)
        layer1_1 = x
        x = self.slice2(x)
        layer2_1 = x
        ...
        x = self.slice6(x)
        layer5_1 = x
        # 输出所有层，包括style representation和content representation放在一起输出了
        vgg_outputs = namedtuple("VggOutputs", self.layer_names)
